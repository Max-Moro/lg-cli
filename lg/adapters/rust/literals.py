...
RUST_STRING = LiteralPattern(
    ...
    tree_sitter_types=["string_literal", "raw_string_literal"],
    opening=_detect_raw_string_opening,
    closing=_detect_raw_string_closing,
    placeholder_position=PlaceholderPosition.INLINE,
    placeholder_template="…",
    interpolation_markers=[],  # Rust format strings can't be reliably detected
)

# Array expressions: [elem1, elem2, ...]
RUST_ARRAY = LiteralPattern(
    category=LiteralCategory.SEQUENCE,
    tree_sitter_types=["array_expression"],
    opening="[",
    closing="]",
...
RUST_VEC_MACRO = LiteralPattern(
    ...
    tree_sitter_types=["macro_invocation"],
    wrapper_match=r"^vec$",  # Match "vec" exactly (without !)
    opening="![",  # Include ! in opening to extract wrapper correctly
    closing="]",
    placeholder_position=PlaceholderPosition.END,
    placeholder_template='"…"',
    min_elements=1,
    comment_name="vec",
    priority=10,  # Higher than generic array
)
...
RUST_HASHMAP_INIT = LiteralPattern(
    ...
    tree_sitter_types=["let_declaration"],  # Match let declarations
    opening="",  # Not used for BLOCK_INIT
    closing="",  # Not used for BLOCK_INIT

    statement_pattern="*/call_expression",  # BLOCK_INIT field: var.insert(...)
