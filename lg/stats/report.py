...
def compute_stats(
    *,
    ...
    for rel, b in dedup.items():
        ...
        t_proc = t_proc_cached if isinstance(t_proc_cached, int) else tokenizer.count_text(b.processed_text)
        ...
        t_raw = t_raw_cached if isinstance(t_raw_cached, int) else tokenizer.count_text(b.raw_text)
    ...
    if not isinstance(t_sections_only, int):
        t_sections_only = tokenizer.count_text(rendered_sections_only_text)
    ...
    if not isinstance(t_pipeline, int):
        t_pipeline = tokenizer.count_text(rendered_sections_only_text)
    ...
    if not isinstance(t_final, int):
        t_final = tokenizer.count_text(rendered_final_text)
