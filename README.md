...
* Общая секционная настройка `skip_empty` (по умолчанию `true`).
* На уровне адаптера можно переопределить `empty_policy`: `inherit` | `include` | `exclude`.

#### Языковые адаптеры (встроенные)
