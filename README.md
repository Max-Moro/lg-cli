...
* **шаблоны контекстов** (вставки секций и вложенные шаблоны),
...
1. **Опишите правила в репозитории**
   Создайте `lg-cfg/config.yaml` и `lg-cfg/contexts/`. В YAML — секции (наборы файлов + фильтры). В `contexts/` — Markdown-шаблоны, куда секции подставляются плейсхолдерами.
...
4. **Фиксируйте удачные промты**
   Шаблоны в `lg-cfg/contexts/**` — это ваши «хорошо работающие» форматы запросов: можно воспроизводить, версионировать, делать варианты для разных задач и агентов.
...
> Важно: каталог конфигурации называется **`lg-cfg/`**.

Структура:

```
lg-cfg/
├─ config.yaml           # основной YAML с секциями
└─ contexts/
   ├─ docs/architecture.tpl.md
   └─ feature-x.tpl.md
```

### `config.yaml`

Ключевая идея — **секции**. Секция описывает:
  ...
  targets:
    ...
    - match: ["/docs/**.md", "/notes/*.md"]
      markdown:
        drop:
          sections:
            - match: { kind: regex, pattern: "^(License|Changelog|Contributing)$", flags: "i" }                        # ветка по родителям → удалить поддерево
```
...
Стоит помнить, что дефолты подобраны максимально универсально. Обычно конфигурации из пары строк хватает, а `targets` нужны только для особо чувствительных мест проекта.
...
#### Markdown позволяет
  ...
  * Локально вырезать **блоки по маркерам**.
  * Снести **YAML front matter** в начале.
  * Вставить **плейсхолдеры** на месте удалённого (по желанию).

*Подробнее:* [Руководство по работе с Markdown](docs/markdown.md).
...
Шаблоны контекстов расположены в `lg-cfg/contexts/**.tpl.md`.

В шаблоне можно:

* вставлять секции: `${core-model-src}`
* включать другие шаблоны: `${tpl:docs/architecture}`
...
LG соберёт один итоговый документ, а также «версию без клея шаблонов» (для расчёта overhead).
...
* `ctx:<name>` — шаблон из `lg-cfg/contexts/<name>.tpl.md`,
* `sec:<name>` — виртуальный контекст одной секции,
* `<name>` — сначала ищется как `ctx:`, иначе как `sec:`.
