...
from .conftest import (
    ...
    create_conditional_template, TagConfig, write_tags_yaml,
    write
)
...
def test_conditional_python_adapter_options(adaptive_project):
    ...
    write_tags_yaml(root, global_tags=special_tags, append=True)
    
    # Создаем структуру пакетов с __init__.py файлами
    write(root / "src" / "__init__.py", "")  # пустой (тривиальный)
    write(root / "src" / "package1" / "__init__.py", "")  # пустой (тривиальный) 
...
"""
    ...
    result1 = render_for_test(root, "ctx:adapter-options-test", make_run_options())
    
    # Проверяем, что тривиальные __init__.py отсутствуют в обеих секциях
    init_markers = [
        "FILE: src/__init__.py",
        "FILE: src/package1/__init__.py"
    ]
    ...
    assert "FILE: src/package2/__init__.py" in result1
    assert "__version__ = '1.0.0'" in result1
    
    # Обычные модули должны присутствовать
    assert "FILE: src/package1/module.py" in result1
    assert "FILE: src/package2/core.py" in result1
    ...
    result2 = render_for_test(root, "ctx:adapter-options-test", options)
    ...
    trivial_init1_count = result2.count("FILE: src/__init__.py") 
    trivial_init2_count = result2.count("FILE: src/package1/__init__.py")
    nontrivial_init_count = result2.count("FILE: src/package2/__init__.py")
    
    # Тривиальные __init__.py должны появиться только один раз (во второй секции)
    assert trivial_init1_count == 1, f"Expected 1 occurrence of src/__init__.py, got {trivial_init1_count}"
    assert trivial_init2_count == 1, f"Expected 1 occurrence of src/package1/__init__.py, got {trivial_init2_count}"
    
    # Нетривиальный должен появиться дважды (в обеих секциях)
    assert nontrivial_init_count == 2, f"Expected 2 occurrences of src/package2/__init__.py, got {nontrivial_init_count}"
...
"""
    ...
    result1 = render_for_test(root, "ctx:multiple-options-test", 
                             make_run_options(extra_tags={"include-inits"}))
    
    assert "FILE: src/__init__.py" in result1  # __init__.py включен
    ...
    result2 = render_for_test(root, "ctx:multiple-options-test",
                             make_run_options(extra_tags={"strip-bodies"}))
    
    assert "FILE: src/__init__.py" not in result2  # __init__.py пропущен
    ...
    result3 = render_for_test(root, "ctx:multiple-options-test",
                             make_run_options(extra_tags={"strip-bodies", "verbose-mode"}))
    
    assert "FILE: src/__init__.py" in result3     # __init__.py включен (verbose-mode)
    ...
    result4 = render_for_test(root, "ctx:multiple-options-test",
                             make_run_options(extra_tags={"include-inits", "strip-bodies", "verbose-mode"}))
    
    assert "FILE: src/__init__.py" in result4     # __init__.py включен
    assert "def public_function():" in result4    # сигнатуры есть  
    assert "internal_logic()" in result4          # тела функций сохранены


def test_conditional_markdown_adapter_options(adaptive_project):
    """
    Тест условных опций Markdown адаптера.
    
    Проверяет динамическое изменение поведения Markdown адаптера
    на основе активных тегов.
    """
    root = adaptive_project
    
    # Добавляем теги для управления Markdown
    md_tags = {
        "preserve-h1": TagConfig(title="Сохранить заголовки H1"),
        "compact-docs": TagConfig(title="Компактная документация")
    }
    write_tags_yaml(root, global_tags=md_tags, append=True)
    
    # Создаем Markdown файлы с различными заголовками
    write(root / "docs" / "guide.md", textwrap.dedent("""
    # Main Guide
    ...
    # Секция с условными Markdown опциями
    sections_content = textwrap.dedent("""
    adaptive-docs:
      extensions: [".md"]
      code_fence: false
      markdown:
        max_heading_level: 2  # по умолчанию убираем H1 и сдвигаем
        when:
          - condition: "tag:preserve-h1"
            max_heading_level: 3  # сохраняем H1, убираем только глубокую вложенность
          - condition: "tag:compact-docs"
            max_heading_level: 1  # агрессивное сжатие заголовков
      filters:
        mode: allow
        allow:
          - "/docs/**"
    ...
    template_content = """# Conditional Markdown Test

${adaptive-docs}
"""
    
    create_conditional_template(root, "markdown-options-test", template_content)
    
    # Тест 1: базовое поведение (max_heading_level: 2)
    result1 = render_for_test(root, "ctx:markdown-options-test", make_run_options())
    
    # H1 должны быть убраны или понижены
    assert "# Main Guide" not in result1 or "## Main Guide" in result1
    assert "# API Documentation" not in result1 or "## API Documentation" in result1
    # H3 должны быть понижены до H2
    assert "### Subsection 1.1" not in result1
    assert "## Subsection 1.1" in result1 or "# Subsection 1.1" in result1
    
    # Тест 2: с preserve-h1 (max_heading_level: 3)
    result2 = render_for_test(root, "ctx:markdown-options-test",
                             make_run_options(extra_tags={"preserve-h1"}))
    
    # H1 должны быть сохранены
    assert "# Main Guide" in result2
    assert "# API Documentation" in result2
    # H3 могут быть сохранены (уровень 3 разрешен)
    
    # Тест 3: с compact-docs (max_heading_level: 1)
    result3 = render_for_test(root, "ctx:markdown-options-test", 
                             make_run_options(extra_tags={"compact-docs"}))
    
    # Все заголовки должны быть максимально сжаты
    # H2 и выше должны отсутствовать или быть понижены
    h2_count = result3.count("## ")
    h3_count = result3.count("### ")
    assert h2_count == 0, f"Expected no H2 headers in compact mode, found {h2_count}"  
    assert h3_count == 0, f"Expected no H3 headers in compact mode, found {h3_count}"
...
def test_conditional_options_with_complex_conditions(adaptive_project):
    ...
    write_tags_yaml(root, global_tags=complex_tags, append=True)
    
    # Создаем файлы
    write(root / "src" / "__init__.py", "# Main package\n")  # не тривиальный
...
"""
    ...
    result1 = render_for_test(root, "ctx:complex-conditions-test",
                             make_run_options(extra_tags={"production"}))
    
    assert "def debug_function():" in result1      # сигнатуры есть
    assert "print(" not in result1                # тела функций убраны
    assert "def production_function():" in result1
    assert "return process_data()" not in result1
    
    # Тест 2: debug режим - все детали
    result2 = render_for_test(root, "ctx:complex-conditions-test", 
                             make_run_options(extra_tags={"debug"}))
    
    assert "FILE: src/__init__.py" in result2     # __init__.py включен
    assert "def debug_function():" in result2     # сигнатуры есть
    assert "print(" in result2                    # тела функций сохранены
    assert "return process_data()" in result2
    
    # Тест 3: api-docs без internal-docs - только сигнатуры
    result3 = render_for_test(root, "ctx:complex-conditions-test",
                             make_run_options(extra_tags={"api-docs"}))
    
    assert "def debug_function():" in result3     # сигнатуры есть  
    assert "print(" not in result3               # тела функций убраны
    assert "return process_data()" not in result3
    
    # Тест 4: api-docs + internal-docs - internal-docs отменяет strip_function_bodies
    result4 = render_for_test(root, "ctx:complex-conditions-test",
                             make_run_options(extra_tags={"api-docs", "internal-docs"}))
    
    assert "FILE: src/__init__.py" in result4     # __init__.py включен (internal-docs)
    assert "def debug_function():" in result4     # сигнатуры есть
    assert "print(" in result4                    # тела функций сохранены (internal-docs приоритетнее)
    assert "return process_data()" in result4
...
def test_conditional_options_inheritance_and_priority(adaptive_project):
    ...
    write_tags_yaml(root, global_tags=priority_tags, append=True)
...
"""
    ...
    result1 = render_for_test(root, "ctx:priority-test",
                             make_run_options(extra_tags={"base-mode"}))
    
    assert "FILE: src/__init__.py" in result1  # skip_trivial_inits: false
    
    # Тест 2: base-mode + override-mode - второе правило переопределяет первое
    result2 = render_for_test(root, "ctx:priority-test", 
                             make_run_options(extra_tags={"base-mode", "override-mode"}))
    
    assert "FILE: src/__init__.py" not in result2  # skip_trivial_inits: true (переопределено)
    
    # Тест 3: все три тега - final-mode имеет высший приоритет
    result3 = render_for_test(root, "ctx:priority-test",
                             make_run_options(extra_tags={"base-mode", "override-mode", "final-mode"}))
    
    assert "FILE: src/__init__.py" in result3  # skip_trivial_inits: false (final-mode)
    
    # Тест 4: только final-mode - не зависит от других правил
    result4 = render_for_test(root, "ctx:priority-test", 
                             make_run_options(extra_tags={"final-mode"}))
    
    assert "FILE: src/__init__.py" in result4  # skip_trivial_inits: false