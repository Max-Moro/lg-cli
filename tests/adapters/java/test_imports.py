"""
Tests for import optimization in Java adapter.
"""
...
class TestJavaImportOptimization:
    """Test Java import optimization policies."""
    ...
    def test_keep_all_imports(self, do_imports):
        ...
        assert meta.get("java.removed.imports", 0) == 0
        assert "import java.util.*;" in result
        assert "import java.time.*;" in result
    ...
    def test_strip_local_imports(self, do_imports):
        ...
        adapter = make_adapter(JavaCfg(imports=import_config))

        result, meta = adapter.process(lctx(do_imports))

        assert meta.get("java.removed.import", 0) > 0

        assert "import java.util.*;" in result
        assert "import org.springframework" in result
