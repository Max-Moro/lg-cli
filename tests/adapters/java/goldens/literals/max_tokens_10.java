...
class LargeConstants {
    ...
    public static final Map<String, String> ERROR_MESSAGES = Map.ofEntries(
        Map.entry("VALIDATION_FAILED", "Input validation failed. Please check you…") // literal string (−4 tokens),
        ...
        Map.entry("RESOURCE_NOT_FOUND", "The requested resource could not be foun…") // literal string (−2 tokens),
        Map.entry("INTERNAL_ERROR", "An internal server error occurred. Please…") // literal string (−3 tokens),
        Map.entry("RATE_LIMIT_EXCEEDED", "Rate limit exceeded. Please wait before makin…") // literal string (−1 tokens),
        Map.entry("INVALID_REQUEST_FORMAT", "The request format is invalid. Please check t…") // literal string (−1 tokens)
    );
}
