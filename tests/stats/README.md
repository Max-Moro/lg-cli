...
Создана полная инфраструктура для тестирования подсистемы статистики и токенизации в Listing Generator. Инфраструктура позволяет:
...
tests/
├── infrastructure/
│   ├── tokenizer_utils.py          # Новые утилиты для токенизаторов
│   └── __init__.py                 # Обновлен с экспортом утилит
│
...
### 3. Утилиты (`tests/infrastructure/tokenizer_utils.py`)

- `create_tokenizer()` - создание TokenService с настройками
- `compare_tokenizers()` - сравнение результатов токенизации
- `get_compression_ratio()` - вычисление сжатия
- `estimate_tokens_for_file()` - оценка токенов в файле
- `count_tokens_with_cache_check()` - подсчет с проверкой кеша
...
- `tests/infrastructure/tokenizer_utils.py` - утилиты для токенизаторов
...
### Обновлено
- `tests/infrastructure/__init__.py` - экспорт новых утилит
