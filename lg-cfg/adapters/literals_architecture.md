...
### 5. Composing Nested Processing

**Принцип**: Placeholders создаются в произвольном порядке, PlaceholderManager автоматически управляет композицией вложенных структур.

**Композиция**: PlaceholderManager сортирует placeholders по ширине (narrower first) и применяет через `add_replacement_composing_nested`, который автоматически композирует вложенные edits.

**Преимущество**: Компоненты не зависят от порядка обработки - вложенные литералы разных типов обрабатываются корректно независимо от порядка создания placeholders.
...
**Единопроходность**: все типы литералов (строки, коллекции, фабрики, блоки) обрабатываются в едином проходе. PlaceholderManager автоматически композирует вложенные структуры независимо от порядка обработки.
...
**Element** — простой текстовый фрагмент (text, raw_text, start_offset, end_offset). Элементы не хранят информацию о вложенности — PlaceholderManager обрабатывает композицию автоматически
...
8. **Упрощенная модель Element** — элементы не хранят информацию о вложенности; PlaceholderManager автоматически композирует вложенные placeholders через сортировку по ширине
