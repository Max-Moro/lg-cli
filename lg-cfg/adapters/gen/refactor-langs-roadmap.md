...
## Этап 2. РRegistry hardening и YAGNI: безопасная работа с незавершёнными адаптерами

### Задачи

- [ ] В `lg/adapters/__init__.py` убрать ленивую регистрацию неготовых языков (`javascript`, `java`, `cpp`, `scala`) до появления реализаций
- [ ] В `lg/adapters/registry.py` добавить безопасный fallback:
  - [ ] Обернуть импорт адаптера в `try/except ImportError`
  - [ ] При неудаче — лог‑предупреждение + регистрация `BaseAdapter` для соответствующих расширений (чтобы не повторять попытки)
- [ ] Добавить тест: наличие файлов `.js/.java/.cpp/.scala` не валит пайплайн, а обрабатывается базовым адаптером

### Критерии готовности

- Пайплайн устойчив при встрече «чужих» файлов
- Нет ленивых регистраций на несуществующие модули

---

## Этап 3. RangeEditor: диагностика конфликтов правок и метрики
...
## Этап 4. Summarize импортов: семантика вместо удаления
...
## Этап 5. DRY для тримминга: единый утиль «truncate_with_ellipsis» и подсчёт оверхеда
...
## Этап 6. Плейсхолдеры и «экономичность»: предсказуемые пороги и отчётность
...
## Этап 7. Анализаторы кода: унификация public API и классов/полей
...
## Этап 8. Unicode‑прочность, снапшоты и property‑based тесты
...
## Этап 9. Документация, примеры и миграции

### Задачи

- [ ] Обновить `docs/` по новым API V2, примеры запросов и имен capture’ов
- [ ] Добавить «рецепты» конфигураций адаптеров (public_api_only, literals, imports)
- [ ] Миграционный гид: чем V2 отличается от V1 и как обновить внешние расширения (если есть)

### Критерии готовности

- Команде понятно, как пользоваться V2 и как писать новые адаптеры

---

## Этап 10. Rollout на новые языки (после стабилизации ядра)

### Задачи

- [ ] Подготовить минимальные адаптеры JavaScript / Java / C++ / Scala
- [ ] Включить ленивую регистрацию только для реально поставляемых модулей
- [ ] Для каждого — базовые тесты запросов и два‑три оптимизатора (imports/comments/bodies)

### Критерии готовности

- Новые адаптеры не нарушают стабильность существующих, тесты покрывают минимум сценариев

---

## Общие правила этапирования

- Каждый этап — PR с чётким скоупом, changelog и bump версии (микро/минор)
- В конце этапа: полный прогон тестов, исправление регрессий, фиксация артефактов CI (coverage, snapshots)
- Совместимость: по возможности сохраняем внешние контракты; при разрывах — миграционный раздел в docs

## Пример последовательности релизов

- v0.3.x → v0.3.(x+1): Этап 1 (V2) — подкапотное изменение, поведение сохранено, но устойчивость выше
- v0.4.0: Этапы 2–4 (registry, RangeEditor, summarize imports) — минор из‑за изменения поведения summarize
- v0.4.1–v0.4.2: Этапы 5–6 (DRY тримминг, плейсхолдеры/бюджет) — полировка
- v0.5.0: Этапы 7–9 — унификация анализаторов, Unicode‑прочность, документация
- v0.6.x: Этап 10 — новые языки

---

## Примечания по реализациям

- Везде, где есть выбор между «падать» и «деградировать предсказуемо», выбираем второе и оставляем след в метриках
- Для Tree‑sitter V2 допускаем временную избыточность тестов (дубликаты) на период миграции
- Отчётность: наращиваем метрики только там, где это помогает дебагу/эксплуатации; не превращать `meta` в свалку
