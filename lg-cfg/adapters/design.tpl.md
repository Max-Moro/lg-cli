## Дизайн функционального блока «Языковые адаптеры»
...
3. **`TreeSitterDocument`** — промежуточное представление для разобранного кода
...
- `strip_external` — удалять внешние пакеты/библиотеки
- `strip_local` — удалять локальные импорты
...
- `max_tokens` — максимальный допустимый размер контента литерала в токенах

#### 6. Бюджетирование токенов (`budget`)

Ограничение общего размера:
- `max_tokens_per_file` — максимальное количество токенов на файл
- `priority_order` — порядок приоритета оптимизаций при превышении бюджета: 
  - импорты внешних пакетов/библиотек (высокий приоритет)
  - оптимизация крупных литералов
  - крупные комментарии (не докстринги)
  - тела в приватных функциях и методах
  - локальные импорты
  - оставление только публичного API
  - тела в публичных функциях и методах (т. е. любые тела)
  - оставление в докстрингах первого предложения (низкий приоритет)

Система бюджетирования никогда по итогу не удаляет полностью документацию и полностью публичные методы. Эти 2 категории остаются, чтобы хоть какую-то информацию иметь о коде.
...
- `inline` — в однострочном комментарии
- `block` — в многострочном комментарии
...
В качестве основного инфраструктурного парсера языков принято решение использовать библиотеку Tree-sitter с соответствующими языковыми грамматиками.

Основной модуль реализации `lg/adapters/tree_sitter_support.py` с абстрактным классом `TreeSitterDocument`. Конкретные языковые адаптеры могут наследовать данный класс с целью подключения своих грамматик, уточнения query выражений и другой кастомизации. 
