...
Опции:
- `strip_trivial_constructors` — пропуск тела в тривиальных конструкторах (только простая инициализация полей)
...
- `priority_order` — порядок приоритета оптимизаций при превышении бюджета: 
  ...
  - тела в приватных методах и содержимое тривиальных конструкторов
  - локальные импорты
  - приватные методы полностью
  - тела в публичных методах
  - тривиальные аксессоры
