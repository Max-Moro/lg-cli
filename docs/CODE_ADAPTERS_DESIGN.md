...
2. Реализовать метод `parse_code()` для конкретного языка
3. При необходимости переопределить методы оптимизации
4. Зарегистрировать через `register_lazy()` в `__init__.py`
