...
**Python** дополнительно предлагает `skip_trivial_inits: true` для пропуска пустых или тривиальных `__init__.py` файлов, и специальные обработчики для декораторов (`@property`, `@classmethod`).

**TypeScript** добавляет `skip_barrel_files: true` для пропуска barrel-файлов (index.ts с реэкспортами), оптимизированную работу с JSX и декораторами, а также специальную логику для экспортируемого API.
