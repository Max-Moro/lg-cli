...
```
lg-cfg/
├─ sections.yaml           # файл секций (может быть в любой поддиректории)
...
> • Для файлов `sections.yaml`:
>   - `lg-cfg/sections.yaml` → секции БЕЗ префикса (например, `docs`, `src`)
>   - `lg-cfg/adapters/sections.yaml` → секции с префиксом `adapters/` (например, `adapters/src`, `adapters/test`)
> • Для фрагментов `*.sec.yaml`:
>   - если фрагмент содержит **ровно одну** секцию → канон-ID = префикс директории + имя секции
>   - иначе — `prefix/section` (где `prefix` = путь фрагмента без суффикса `.sec.yaml`)
> Это позволяет делать короткие ссылки: из `lg-cfg/adapters/overview.ctx.md` можно писать `${src}` вместо `${adapters/src}`.
