...
3. **Переиспользование оценщика условий**: Система использует существующий оценщик условий из основного шаблонизатора, обеспечивая единообразие поведения (`self.template_ctx.evaluate_condition(node.condition_ast)`).
