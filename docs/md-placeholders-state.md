...
#### [] Итерация 3: Состояние виртуальных секций в пайплайне LG

- Корректировка хранения состояния виртуальных секций в `lg/template/processor.py`. Перенос состояния из `_virtual_sections_cache` в TemplateContext, чтобы проще было его достать в остальном пайплайне.
- Проброс состояния виртуальных секций через `SectionProcessor` до `build_section_manifest`. Если состояние перенести в TemplateContext, то этот пункт закрывается автоматически.
- Склейка состояния виртуальных секций в `build_section_manifest` с обычными YAML манифестами от `load_config(scope_dir)`
